(function(window) {
  ...
  function Voxel() {
    ...
    function SetMesh(_mesh) {
      ...
      if (_mesh === undefined || _mesh.constructor !== voxelcss.Mesh) {
        return;
      }
      ...
      if (!!old) {
        old.removeEventListener('change', OnMeshChange);
      }
    ...
    function UpdateLightSource(lightSources) {
      ...
      for (i = 0, lightSource; lightSource = lightSources[i++];) {
        brightness = lightSource.getBrightness();
        scale      = brightness[1] - brightness[0];
        shift      = 1 - brightness[1];
        ...
        if (back > 0) {
          result = AngleFromLightSource(x, y, z, {
            A: 0,
            B: 0,
            C: -1
          });
    ...
    function Rotate(point, rotMatrix) {
      ...
      var rotated       = MultiplyMatrices(rotMatrix, column_vector);
    ...
    function ApplyMesh() {
      ...
      for (label in faces){
        ...
        if (faceMesh instanceof voxelcss.ImageFace){
          ...
          faceElem = faces[label].parentElement;
